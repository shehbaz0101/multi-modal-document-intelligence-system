...
class ParserSettings(BaseSettings):
    ...
    primary: str = "cpu"              # cpu | docling | llamaparse
...
class EmbeddingSettings(BaseSettings):
    ...
    text_model: str = "all-MiniLM-L6-v2"   # fast on CPU
    text_dim: int = 384
    text_batch_size: int = 64
    visual_model: str = "vidore/colqwen2-v1.0"
    visual_enabled: bool = False             # needs GPU — leave False
    # Gemini model used for figure captioning
...
class GenerationSettings(BaseSettings):
    ...
    # gemini-2.0-flash : fast, cheap, vision-capable — good default
    # gemini-2.5-pro   : best quality, use for finance/legal/medical
    model: str = "gemini-2.0-flash"
    max_tokens: int = 1500
    temperature: float = 0.1
    api_key: str | None = None         # reads GEMINI_API_KEY from env
